<template>
  <div>
    ...
    <p v-if="!isDefault">
      <input type="checkbox" :disabled="user.attrs.perm_admin" v-model="user.lockPassword"> {{ $t('settings.lockPassword') }}
    </p>
...
export default {
  ...
  watch: {
    'user.attrs.perm_admin': function () {
      if (!this.user.attrs.perm_admin) return
